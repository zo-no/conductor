...
- `@conductor/web` — 面向人类用户的 Web UI（当前生产用）
- `@conductor/web-next` — 基于 Next.js 的 Web UI（实验性，开发中）
...
HTTP API 默认无鉴权（适合本地开发）。启用后，所有 `/api/*` 路由需要携带访问令牌。

令牌存储在 `~/.conductor/auth.json`（权限 600）。

支持三种传递方式：
- **HTTP Header**：`Authorization: Bearer <token>`
- **Cookie**：`conductor_token=<token>`
- **Query param**：`?token=<token>`（适用于 SSE，EventSource 不支持自定义 header）

CLI 命令：
- `conductor auth token` — 生成随机 64 位十六进制令牌并保存
- `conductor auth status` — 查看当前鉴权状态
- `conductor auth disable` — 禁用鉴权，删除 `auth.json`

Web UI：启用鉴权后访问时显示登录页面，令牌存储在 `sessionStorage`（关闭标签页后清除）。
