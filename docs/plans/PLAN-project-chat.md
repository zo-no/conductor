...
- 每次请求把最近 **40 条**消息传给 AI（足够上下文，不撑爆 token）
- 超过 40 条的历史只在 UI 展示，不传给 AI
- 不做摘要，保持简单
...
对话第一条用户消息发送后，在 AI 回复完成时，**额外发一个轻量请求**让 AI 生成标题：

```
根据以下对话内容，用 10 个字以内生成一个简洁的标题，只返回标题文字，不要加引号或其他内容。

用户：{firstUserMessage}
```

拿到标题后更新 `project_chats.title`，设 `auto_rename = 0`。
